...
def main():
    ...
    def insert_to_CodeBox(text, history_id=None, clear=False):
        if clear:
    ...
    def send():
        ...
        status.configure(text="●  Status:  Getting info...", text_color="#facc15")
    ...
    def show_model_result(result):
        insert_to_CodeBox(result["code"], History[-1]["ID"], True)
    ...
    SavedCommandsBox = CTkScrollableFrame(
        SavedFrame,
        fg_color="#0d1424",
        width=210,
        height=225,
    ...
    RightBorder2 = CTkFrame(
        SavedFrame,
        width=2,
        height=230,
        fg_color="#1e293b"
    )
    RightBorder2.place(x=230, y=328)
    ...
    CopyCodeBtn = CTkButton(
        ...
        font=("Segoe UI", 12),
        corner_radius=8,
        command=lambda: pyperclip.copy(CodeBox.get("1.0", "end-1c"))
    )
    ...
    ModelMenu.configure(values=available_models)

    if "qwen3:8b" in available_models:
        ModelMenu.set("qwen3:8b")
    elif available_models:
        ModelMenu.set(available_models[0])
